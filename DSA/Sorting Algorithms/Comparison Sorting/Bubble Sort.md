It is a simple sorting algorithm in which `ith` index value is compared with it's next index value i.e., `i+1` and if founded larger than the next value than a swap is performed. With each iteration the size of the comparison keep getting smaller and values keep getting arranged from right side having the larger value and then never compared again.
